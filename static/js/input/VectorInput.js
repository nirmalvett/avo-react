...
export default class VectorInput extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        let error = this.validateVector();
        return (
            <div>
                <TextField value={this.state.answer} label='Enter vector' onChange={(e) => this.onChange(e)}
                           error={!Array.isArray(error)} helperText={Array.isArray(error) ? undefined : error}/>
                <br/><br/>
                {Array.isArray(error) ? getMathJax('\\(\\begin{bmatrix}' + error.map(y => '\\text{' + y + '}').join('\\\\') + '\\end{bmatrix}\\)', 'body1') : undefined}
            </div>
        );
    }
    ...
    validateVector() {
        let matrix = this.state.answer.replace(/ /g, '');
        if (matrix.length === 0)
            return 'No answer given';
        let cells = matrix.split(',');
        let validation = cells.map(cell => this.validate(cell));
        validation = validation.map((cell, c) => cell === false ? null :
            '(' + (c+1) + ') ' + cell);

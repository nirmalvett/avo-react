...
import {
    Popper, Card, List, Divider, Tooltip, Paper, MenuItem, TextField, Button, Collapse, ListItem, CardHeader,
    IconButton, Typography, ListItemText, ListSubheader, ListItemSecondaryAction, Tabs, Tab, Select, Input,
...
export default class ManageClasses extends Component {
    ...
    tests; classSize; classMedian; classAverage; standardDeviation; topMarkPerStudent; testMean; testMedian; testSTDEV;
    totalMark; submitted; grade; topMarksPerStudent; enrollKey;
    ...
    detailsCard() {
        ...
        if (this.state.c !== null) // If a class is selected
            ...
            return ManageClasses.detailsCard_nothingSelected();
    }
    ...
    detailsCard_selectedTest(analyticsDataObj, selectedTest, uniqueKey1) {
        return (
            <Fragment key={`detailsCard-${uniqueKey1}`}>
                ...
                {this.detailsCard_selectedTest_attempts(analyticsDataObj, selectedTest, uniqueKey1)}
            </Fragment>
        );
    }
                                ...
                                <span style={{marginLeft: '1.0em', marginRight: '1.0em'}}>
                                    <FormControl>
                                        ...
                                        <Select
                                            ...
                                            onChange={e => this.setState({testStatsDataQuestionIdx: e.target.value})}
    ...
    detailsCard_selectedTest_attempts(analyticsDataObj, selectedTest) {
        if (this.state.activeTab === CONST_TAB_MY_ATTEMPTS) return (
            <Fragment>
                <br/>
                <List style={{flex: 1, overflowY: 'auto', overflowX: 'hidden'}}>
                    {/* Show all the students that are in the class*/}
                    {this.state.results.map((x, idx) =>
                        <ListItem key={`Student-Card-index:${idx}`} disabled={x.tests.length === 0}>
                            {x.tests.length === 0
        ...
        return (
            <Tooltip title="Edit marks for selected attempt">
                <IconButton
                    ...
                    disabled={x.tests.length === 0}
                    onClick={() =>
                        this.props.markEditor(this.state.results[idx].tests[this.state.resultsIndexArray[idx]].takes)
                    }
                >
                    <EditOutlined/>
                </IconButton>
    ...
    getTestCardGraphSeries() {
        ...
        if (this.state.testStatsDataSelectIdx === 0) {
            ...
            let myBestMark = 0;
            selectedTest.submitted.forEach(obj => myBestMark = obj.grade > myBestMark ? obj.grade : myBestMark);

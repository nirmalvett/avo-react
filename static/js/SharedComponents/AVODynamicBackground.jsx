...
export default class AVODynamicBackground extends Component {
	...
	render() {
		return (
			<div style={{ position : 'relative', width: '-webkit-fill-available', height: '-webkit-fill-available' }}>
				<canvas id="glscreen" style={{ position : 'absolute', height : 'inherit', width : 'inherit' }}></canvas>
				{this.props.children}
			</div>
		);
	};
	...
	componentDidMount() {
		...
		let render = () => {
			...
			currentTime = (now - startTime) / 20000; // update the current time for animations

...
export default class AVOBarChart extends Component<AVOBarChartProps> {
    storedDataPoints: never[][];
    shouldUpdate: boolean;

    constructor(props: AVOBarChartProps) {
        super(props);
    ...
    checkIfPointExists(i: number, j: number, value: number) {
    ...
    render() {
        ...
        for (let i = 0; i < numPoints; i++) {
            ...
            for (let j = 0; j < currentSet.length; j++) {
                ...
                bars.push(
                    <path
                        className='avo-bar-chart-bar'
                        d={`M${x1} ${y1} L${x1} ${y2} L${x2} ${y2} L${x2} ${y1} Z`}
                        fill={props.colors[j]}
                    />,
                );
            }
                    ...
                    <div
                        ...
                        onMouseMove={() => {
                            ...
                            props.labels.forEach((obj, index) => {
                                tooltipEL.innerHTML += `<div style='margin-top: 4px'><span style='float: left; height: 9px; width: 9px; border-radius: 50%; background: ${props.colors[index]}; margin-top: 3px;'/><b style='margin-left: 4px'>${obj}:</b>&nbsp;&nbsp;${currentSet[index]} <br/></div>`;
                            });
                        }}
                        onMouseLeave={() => {
                            const tooltipEL = document.getElementById('avo-bar-chart-tooltip');
            ...
            yAxisTooltip.push(
                <line
                    ...
                    strokeWidth='0.5'
                    stroke='#fafafa'
                    x1={15 - currentSet.length + maxStep * i + maxStep / 2}
                    x2={15 - currentSet.length + maxStep * i + maxStep / 2}
                    y1='0'
                    y2='100'
                />,
            );

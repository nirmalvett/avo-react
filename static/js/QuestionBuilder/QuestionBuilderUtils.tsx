...
for (let f in FUNCTIONS)
    if (FUNCTIONS.hasOwnProperty(f)) {
        ...
        let arg_string = Array(arg_count)
            .fill(0)
            .map((_, i) => '&' + i)
            .join(', ');
        if (FUNCTIONS[f][1] === null) FUNCTIONS[f][1] = f + '(' + arg_string + ')';
        if (FUNCTIONS[f][2] === null)
            FUNCTIONS[f][2] = '\\text{' + f + '}\\left(' + arg_string + '\\right)';
    }
let formatRegex = (array: string[]) =>
    array.map(x => x.replace(/[\[\]\-\\.+*?^$(){}=!<>|:]/g, '\\$&'));
const unary_regex =
    '\\$\\w+|@\\d+|\\d+(?:\\.\\d+)?|[\\)\\]\\}]|' +
    formatRegex(
        Object.keys(CONSTANTS).concat(Object.keys(OPERATORS).filter(x => OPERATORS[x][7] === null)),
    ).join('|');
...
export function buildPlainText(mathCode: string): string[] {
    ...
    for (let c in CONSTANTS) if (CONSTANTS.hasOwnProperty(c)) constants[CONSTANTS[c][0]] = c;
    ...
    for (let f in FUNCTIONS)
        if (FUNCTIONS.hasOwnProperty(f))
            functions[FUNCTIONS[f][0]] = [
                f,
                FUNCTIONS[f][3] === '' ? 0 : FUNCTIONS[f][3].split(',').length,
            ];
 ...
 */
export function buildMathCode(
    text: string,
): ReactElement | {mathCode: string; formattedPT: string; LaTeX: string} {
    if (/^ *$/.test(text)) return <Typography color='error'>No expression given</Typography>;
    ...
    for (let i = 0; i < tokens.length; i++) {
        if (
            new RegExp('^(?:' + REGEX + ')$').test(tokens[i]) ||
            /^-?\d+(?:\.\d+)?$/.test(tokens[i])
        ) {
            if (typeof str[str.length - 1] === 'string')
                str[str.length - 1] += ' ' + tokens[i].replace(/_/g, ' ').replace(/-+/g, '-');
            else str.push(tokens[i]);
        } else
            str.push(
                <span style={{color: createMuiTheme().palette.error.main}}> {tokens[i]} </span>,
            );
    }
    if (str.length > 1 || typeof str[0] !== 'string') return <Typography>{str}</Typography>;

    let brackets = 0;
    let operations: [string, number][] = [];
    for (let i = 0; i < tokens.length; i++) {
        let token = tokens[i];
        if (/^-?\d+(?:\.\d+)?$/.test(token)) tokens[i] = [token, token, token, 8, 8];
        else if (/^\$\w+$/.test(token))
            tokens[i] = [token, token, '\\color{DarkOrange}{' + token + '}', 8, 8];
        else if (/^@\d+$/.test(token))
            tokens[i] = [
                '@' + (Number(token.substr(1)) - 1),
                token,
                '\\color{Green}{' + token + '}',
                8,
                8,
            ];
        else if (token in CONSTANTS) tokens[i] = [...CONSTANTS[token], 8, 8];
            ...
            operations.push(tokens[i]);
        } else if (/[)}\]]/.test(token)) brackets -= 10000;
        else if (token !== ',' && token !== ';' && token !== ':')
            alert("Something went wrong when handling this token: '" + token + "'");
        if (brackets < 0) return <Typography color='error'>Mismatched close bracket</Typography>;
    }
    if (brackets !== 0) return <Typography color='error'>Mismatched open bracket</Typography>;
    ...
    for (let i = 0; i < operations.length; i++) {
        ...
        if (o in OPERATORS) {
            ...
            if (data[7] !== null) {
                if (
                    pos === tokens.length - 1 ||
                    !Array.isArray(tokens[pos + 1]) ||
                    tokens[pos + 1].length !== 5
                )
            ...
            for (let i = 0; i < args.length; i++)
                result[2] = (result[2] as string).replace(
                    '&' + i,
                    (tx_p[i] as number) > args[i][4]
                        ? '\\left(' + args[i][2] + '\\right)'
                        : args[i][2],
                );
            tokens[pos] = result;
            ...
            while (tokens[pos + 1] !== ')') {
                if (args.length === signature.length) {
                    ...
                    if (tokens[pos + 1] === ',' && tokens[pos + 2] !== ')')
                        ...
                        return (
                            <Typography color='error'>Can't parse arguments in function</Typography>
                        );
                } else if (tokens[pos + 1] === ',') {
                    tokens.splice(pos + 1, 1);
                ...
                } else return <Typography color='error'>Invalid argument in function</Typography>;
            }
            tokens.splice(pos + 1, 1);
            ...
            while (tokens[pos + 1] !== '}') {
                if (Array.isArray(tokens[pos + 1]) && tokens[pos + 1].length === 5) {
                    ...
                    if (tokens[pos + 1] === ',' && tokens[pos + 2] !== '}')
                        ...
                        return (
                            <Typography color='error'>Can't parse arguments in basis</Typography>
                        );
                } else return <Typography color='error'>Invalid argument in basis</Typography>;
            }
            tokens.splice(pos + 1, 1);
            tokens[pos] = [
                args.map(x => x[0]).join(' ') + ' ' + args.length + ' }',
                '{' + args.map(x => x[1]).join(', ') + '}',
                '\\left\\{' + args.map(x => x[2]).join(',') + '\\right\\}',
                8,
                8,
            ];
        } else if (o === '[') {
            ...
            for (let x = pos + 1; tokens[x] !== ']'; x++) {
                if (tokens[x] === ';') rows += 1;
                else if (Array.isArray(tokens[x]) && tokens[x].length === 5) cols += 1;
            }
            if (cols % rows !== 0)
                return (
                    <Typography color='error'>
...
export function formatString(str: string, strings: {LaTeX: string}[]) {
    ...
    for (let m = re.exec(str); m !== null; m = re.exec(str)) {
        ...
        if (m[0] === '{{') result += '{';
            ...
            result +=
                '\\color{#5599ff}{' +
                strings[Number(m[1])].LaTeX.replace(/\\color{.*?}/g, '') +
                '}';
    }
    return result + str;
}
...
export function formatStringForEditing(str: string, strings: {expr: string}[]) {
    ...
    for (let m = re.exec(str); m !== null; m = re.exec(str)) {
        ...
        else result += '`' + strings[Number(m[1])].expr + '`';
    }
...
export function validateString(str: string) {
    ...
    for (let m = re.exec(str); m !== null; m = re.exec(str)) {
        ...
        if (x.mathCode === undefined) errors.push(x);
    }
    return errors;
}
...
export function initOld(string: string) {
    ...
    for (i = 0; i < steps.length && !steps[i].includes('_') && !steps[i].includes('%'); i++) {
        let step = steps[i]
            .split(' ')
            .map(x =>
                /^\$\d+$/.test(x) ? '$' + (Number((/^\$(\d+)$/.exec(x) as string[])[1]) + 1) : x,
            )
            .join(' ');
    ...
    for (; i < steps.length && !steps[i].includes('%'); i++) {
        let str = steps[i]
            ...
            .join(' ');
        if (str.endsWith(' _A')) str = str.slice(0, -3);
...
export function compile(state: any) {
    ...
    let PART2: string[] = [];
    let PART3 = [editorPrompt]
        .concat(editorPrompts)
        .map(x => compileStringList(x.prompt, x.strings, PART2));
    ...
    let PART7 = editorCriteria.map((x: {explanation: string; strings: {mathCode: string}[]}) =>
        compileStringList(x.explanation, x.strings, PART2),
    );
    let PART8 = editorMath
        .filter((x: {mathCode: string}) => x.mathCode)
        .map((x: {comment: string}) => x.comment);
...
function compileStringList(string: string, strings: {mathCode: string}[], outputStrings: string[]) {
    ...
    for (let m = re.exec(string); m !== null; m = re.exec(string)) {
        ...
        if (m[0] === '{{' || m[0] === '}}') result += m[0];
        else {
            let {mathCode} = strings[Number(m[1])];
            if (!mathCode.includes('_')) mathCode += ' _A';

...
export function renderHints(currentlyEditing: null | string) {
    if (currentlyEditing === null)
        return (
            <Typography>
                Hints will appear here when you start editing something. We are constantly working
                to improve them, so let us know if any part of the question builder is unclear and
                we'll do our best to make it right!
            </Typography>
        );
    if (currentlyEditing === 'mainPrompt')
        return (
            <Typography component='span'>
                <p>
                    Here is where you can enter the main prompt for your question. It will appear
                    before all the answer fields, and be the most heavily emphasized.
                </p>
                <ol>
    ...
    if (currentlyEditing.startsWith('prompt'))
        return (
            <Typography component='span'>
                <p>
                    Here is where you can customize the answer fields for your question. Just choose
                    an answer type, and then enter a prompt. For multiple choice questions, put the
                    prompt on the first line, and each consecutive answer on its own line.
                </p>
                <ol>
                    <li>\(1+1\) is an inline equation. (ctrl+d to insert)</li>
                    <li>\[1+1\] is a block equation. (ctrl+e to insert)</li>
    ...
    if (currentlyEditing === 'math')
        return (
            <Typography component='span'>
                ...
                <p>
                    See the documentation tab in the sidebar for the list of available operators and
                    functions.
                </p>
            </Typography>
        );
    if (currentlyEditing.startsWith('criteria'))
        return (
            <Typography>
                Here, you can set the criteria the question will use to mark students' responses.
                The number of points a part is worth can be any number from 0.01 to 99.99, and the
                criteria expression works the same as any other math expression in the builder. If
                you need to mark a true or false question, look at the documentation on the tf and
                mc functions.
                <br />
                <br />
...
export function Preview(props: PreviewProps) {
    ...
    let varList = [];
    for (let v in state.preview.variables)
        if (state.preview.variables.hasOwnProperty(v))
